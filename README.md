...
- **SQLite** база данных с таблицами:
  ...
  - `Teams` - команды
  - `FUsers` - пользователи Free режима
  - `Judges` - судьи

⚠️ **ВАЖНО: База данных не отслеживается в Git**
- Файл базы данных исключен из репозитория через `.gitignore`
- База данных на сервере управляется отдельным сервисом (Discord бот)
- При деплое база данных **НЕ перезаписывается** - все скрипты деплоя защищены от этого
- Путь к базе данных на сервере: `/var/www/vhosts/194.32.140.220.nip.io/ProdB/database.db`
